...
class automate_hal_test:
    ...
    def enrichWithAuthDB(self):
        ...
        For each author in the user-defined database, the HAL-id and Affiliation ID in HAL will be used to replace the corresponding fields in 
        self.auths.
        """
        auths = self.auths
        for item in auths:
    ...
    def process_paper_ite(self, doc):
        ...
        # Verify if the paper is already in HAL.
            ...
            docTei = auto_hal.produceTeiTree(doc, dataTei)
            xml_path = auto_hal.exportTei(docTei)
    ...
    def verify_if_existed_in_hal(self, doc):
        ...
        if idInHal[0] > 0:
            ...
            if titleInHal[0] > 0:
                auto_hal.addRow(self.docid, 'already in hal', '', 'ids match', titleInHal[1])
                return True
...
if __name__ == '__main__':
    # search_query = 'AU-ID(55659850100) OR AU-ID(55348807500) OR AU-ID(7102745133) AND PUBYEAR > 2018 AND PUBYEAR < 2025'
    ...
    auto_hal = automate_hal_test(perso_data_path, author_db_path, stamps)    
